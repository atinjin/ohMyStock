...
### 🟢 단위 일관성(통화) — 완료 ([spec](superpowers/specs/2026-06-20-currency-consistency-design.md) · [plan](superpowers/plans/2026-06-20-currency-consistency.md))
**USD 기준 통일 + ≈원 참고**:
- [x] 시스템 기준 통화 명시 — `Config.currency="USD"`, `full_report`·페이퍼 state에 `currency` 포함
- [x] 환율 처리 — `GET /api/fx`(USD/KRW, yfinance `USDKRW=X` 재사용, 300초 캐시) + ≈원 참고
- [x] 대시보드 통화 표기 통일 — 백테스트 최종자산·페이퍼 금액·자본 입력을 `$` + `≈ …만원`(LivePreview·시장개요·TOSS와 일치)
- [ ] (후속) KRW 기준 모드/통화 선택 옵션
...
| 실주문 체결 미검증 | 주문 경로만 확인(평일 장중 미체결) | §4 KIS/TOSS 후속 |

> 해소됨: PaperBroker 상태 영속화(SQLite), KIS 어댑터 실연동(모의투자 실 API 검증), 통화 일관성(USD 기준 통일 + ≈원).
